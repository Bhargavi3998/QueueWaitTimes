...
class BackfillPolicy(Policy):
    ...
    def __init__(self, config, start_time, nodes):
        ...
        logging.debug("Scheduler initialized with job_queue_mapping.")
...
class Scheduler:
    ...
    def submit(self, job_queue_pairs):
        def priority_reset(new_job):
            """
            Initialize the job priority.

            :param new_job: Job that needs to be initialized.
            :return: Adjusted priority based on the job's wait time.
            """
        ...
        for job, queueName in job_queue_pairs:  # job_queue_pairs is a list of (job, queueName) tuples
            # Calculate priority
            ...
            # Update the job to queueName mapping
            self.job_queue_mapping[job.job_id] = queueName

        # Sort the pending queue based on priority
        if len(job_queue_pairs) != 0:
    ...
    def run(self, time, job_id=None):
        job_end = False
        job_return = None
        for _ in range(int(time.total_seconds() / int(self._mode))):
            ...
            for new_job in new_submit_jobs:
                # Remove this job from the pending queue because it is now submitted
                self._pending_queue.remove(new_job)

                # Update the queue time and manage the fixed time window
            ...
            # Update the priority for items in the pending queue based on elapsed simulation time
            for item in self._pending_queue:
                item.priority += self._config["PriorityWeightAge"] * int(self._mode) / (
                            float(self._config["PriorityMaxAge"]) * 24 * 60 * 60)

            # Run the cluster for the current time step and update the policy with the completed jobs
            ...
            # Check if a specific job has ended, if a job_id is provided
